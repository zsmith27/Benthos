...
#'@export
#'
select_metrics <- function(long.df, master.df, taxa.rank,
                        metrics.vec = "ALL",
                        tv.col = "BIBI_TV",
                        ffg.col = "BIBI_FFG",
                        hab.col = "BIBI_HABIT",
  ...
  names(long.df) <- names(long.df) %>% toupper() %>% trimws()
  ...
  if(taxa.rank %in% "FAMILY") taxa.rank.df <- family.wide
  if(taxa.rank %in% "GENUS") taxa.rank.df <- genus.wide
  if(!(taxa.rank %in% c("FAMILY", "GENUS"))) taxa.rank.df <- wide(long.df, taxa.rank)
